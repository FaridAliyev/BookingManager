...
public class FlightService {
    ...
    public Optional<Flight> getFlightByCode(String code) {
        return flightDao.getAll().stream()
                .filter(f -> f.getCode().equals(code) && f.getDeparture().isAfter(LocalDateTime.now()))
                .findAny();
    }

    public List<Flight> filterFlights(String city, LocalDate date, int ticketCount) {
        return flightDao.getAll().stream()
                .filter(f -> f.getTo().name().toLowerCase().equals(city) && f.getDeparture().toLocalDate().equals(date)
                        && f.getCapacity() - f.getPassengers().size() - ticketCount >= 0 && f.getDeparture().isAfter(LocalDateTime.now()))
                .sorted(Comparator.comparing(Flight::getDeparture))
                .collect(Collectors.toList());
    }
}
